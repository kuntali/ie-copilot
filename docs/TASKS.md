...
**Last updated:** 2026-09-04 16:43 +08:00
...
| P1-03 | 固化 `uv.lock` 与可重复依赖安装 | DONE | 2026-09-04 16:29 +08:00 | `uv.lock` generated by uv resolver in CI and committed by `github-actions[bot]` at `8e75c934...`. CI now runs `uv lock --check` and `uv sync --frozen`. Run #58 (`33853968125`) shows Check lockfile + frozen Install dependencies success on Python 3.10 and 3.13; Ruff failure is outside this task. |
...
| P1-05 | Unit pytest 全绿且不依赖外部 API | IN_PROGRESS | 2026-09-04 16:43 +08:00 | Run #77 already shows Tests step success on Python 3.10 and 3.13; now formally verify unit-suite boundaries/no external service dependency under a task-specific plan. |
